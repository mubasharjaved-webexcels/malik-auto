  ...
  <div class="main-panel">
    ...
    <div id="dynamic-danger" class="alert alert-danger alert-dismissible fade show d-none" role="alert">
      ...
      <div class="row px-3 py-4">
        <div class="col-sm-12">
          <div class="card">
            {{-- <div class="text-bg-light card-header d-flex justify-content-between align-items-center"> --}}
            <div class="text-bg-light pt-2 px-3 pb-0 card-header d-flex justify-content-between align-items-center">
              ...
              <div class="d-flex align-items-center">
                <form action="{{ route('car-expense.search') }}" method="GET" class="d-flex align-items-left" style="margin-bottom: 5px;">
                  <input type="text" 
            ...
            <div class="card-body">
             ...
             <div class="table-responsive">
                <table id="carExpensesTable" class="table table-striped table-bordered w-auto" style="width: auto; min-width: 100%; overflow-x: visible;">
                  ...
                  <tbody id="car-list">
                    @include('dashboards.car_expenses.partials.expense_listing')
                  </tbody>
                ...
                @if ($carExpenses->hasPages())
                    <div class="mt-4 d-flex justify-content-center">
                        {{ $carExpenses->links('pagination::bootstrap-5') }}
                    </div>
                @endif
              </div>
            </div>
      ...
      <div class="modal fade" id="addCarExpenseModal" tabindex="-1" aria-labelledby="addCarExpenseModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-md">
          <div class="modal-content">
            ...
            <form id="addCarExpenseForm" method="POST" action="{{ route('car-expenses.store') }}">
              @csrf
              <div class="modal-body">
                <div class="row">
                    ...
                    <div class="form-group mb-3">
                      ...
                      <select class="form-select text-dark" id="car_profile_id" name="car_profile_id" required>
                        ...
                        @foreach($carProfiles as $car)
                          <option value="{{ $car->id }}" 
                                  data-rec-no="{{ $car->rec_no }}"
                                  data-chassis="{{ $car->chassis }}"
                                  data-image="{{ $car->car_image ? asset('storage/' . $car->car_image) : asset('images/no-image.png') }}">
                            {{ $car->rec_no }}
                          </option>
                        @endforeach
                      </select>
                  ...
                  <div class="col-md-12">
                    <div class="form-group mb-3">
                      <label for="amount" class="form-label text-dark">Amount <span class="text-danger">*</span></label>
                      ...
                      <select class="form-select" id="edit_car_profile_id" name="car_profile_id" required>
                        ...
                        @foreach($carProfiles as $car)
                          <option value="{{ $car->id }}"
                                  ...
                                  data-chassis="{{ $car->chassis }}"
                                  data-image="{{ $car->car_image ? asset('storage/' . $car->car_image) : asset('images/no-image.png') }}">
                            {{ $car->rec_no }} - {{ $car->chassis ?? 'N/A' }}
                          </option>
                        @endforeach
                      </select>
                      <div class="invalid-feedback"></div>
                      
                      {{-- Selected Car Preview for Edit --}}
...
$(document).ready(function() {
    ...
    $(document).on('click', '.edit-car-expense', function() {
        ...
        $.ajax({
            url: "{{ route('car-expenses.show', ':id') }}".replace(':id', id),
            method: 'GET',
            success: function(response) {
                ...
                $('#editCarExpenseForm').attr('action', "{{ route('car-expenses.update', ':id') }}".replace(':id', id));
    ...
    $(document).on('click', '.delete-car-expense', function() {
        ...
        if(confirm('Are you sure you want to delete this car expense?')) {
            $.ajax({
                url: "{{ route('car-expenses.destroy', ':id') }}".replace(':id', id),
                method: 'DELETE',
                data: {
                    _token: '{{ csrf_token() }}'
                },
...
// Filter by Country, Manager & Record No
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchFilter(data = {}) {
      fetch("{{ route('car-expenses.filter') }}", {
          method: "POST",
          headers: {
