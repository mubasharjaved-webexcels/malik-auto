    ...
    $('#recno-filter').prop('disabled', true);
      ...
      function fetchCarProfiles() {
        ...
        $.ajax({
          ...
          // Retain currently selected values
          ...
          // Enable yard-filter if country is selected
          if (countryId) {
          ...
          // Enable recno-filter if yard is selected
          if (yard) {
      ...
      // Trigger against country changed
      ...
      // Trigger against yard changed
      $('#yard-filter').on('change', function () {
          $('#recno-filter').empty().append('<option value="">Select Rec No</option>');
          fetchCarProfiles();
      });

      // Trigger against rec_no changed
      $('#recno-filter').on('change', function () {
          fetchCarProfiles();
      });

      // Trigger against car status changed
