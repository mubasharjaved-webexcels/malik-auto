...
class CarProfileController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($user->hasRole('admin')) {
            ...
            $carProfiles = $query->with(['countryCurrency', 'expenses.currencyInfo'])->withSum('expenses', 'usd_amount')
                            ->latest()
                            ->paginate($perPage);
            return view('dashboards.car_profiles.index', compact('carProfiles', 'countries' ,'carStatuses'));

        }elseif($user->hasRole('manager')) {
            $countries = Country::where('id', $user->country_id)->get();
            $query = CarProfile::where('assigned_manager_id', $user->id)->where('country_id', $user->country_id);
            ...
            $carProfiles = $query->latest()->paginate($perPage);
            return view('dashboards.car_profiles.index', compact('carProfiles', 'countries' ,'carStatuses'));
